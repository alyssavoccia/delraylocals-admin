...
const Community = () => {
  const { organizations, dispatch } = useContext(DelrayLocalsContext);
  ...
  const onDelete = e => {
    const organizationName = e.target.parentNode.parentNode.parentNode.id;
    db.collection("organizations").where("name", "==", organizationName).get()
      .then(querySnapshot => {
        querySnapshot.docs[0].ref.delete();
      });
    
    const updatedArr = organizations.filter(organization => organization.name !== organizationName);

    dispatch({ type: 'UPDATE_ORGANIZATIONS', payload: updatedArr });
  };

  return (
    <section className="pl-[80px] pr-4">
      <div className="flex justify-between items-center">
        <h1 className="py-4 font-black text-2xl">Organizations</h1>
        <button onClick={() => dispatch({ type: 'OPEN_ORGANIZATIONS_MODAL' })} className="bg-slate-500 hover:bg-slate-600 transition-all text-white font-semibold p-2 rounded flex justify-center w-[150px]">Add Organization</button>
        <GetInvolvedModal />
      </div>
          ...
          <div className="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
            <div className="inline-block min-w-full overflow-hidden rounded-lg shadow">
              <table className="min-w-full leading-normal">
                ...
                <tbody>
                  {
                    currOrganizations.map(organization => (
                      <tr key={organization.name} id={organization.name}>
                        <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                          <p className="whitespace-no-wrap">{organization.name}</p>
                        </td>
                        <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                          <p className="whitespace-no-wrap">{organization.type}</p>
                        </td>
                        <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                          <p className="whitespace-no-wrap">{organization.website}</p>
                        </td>
                        <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                          <p className="whitespace-no-wrap">{organization.phone}</p>
                        </td>
                        <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
